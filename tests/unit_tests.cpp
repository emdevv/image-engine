...
TEST(NativeCPU, RotateOp) {
  ...
  RotateOp op(90);
  out = exec.execute(img, op);

  EXPECT_EQ(out.width, 1);
  EXPECT_EQ(out.height, 2);

  EXPECT_EQ(out.pixels[0], 1); // Pixel 0 R
  EXPECT_EQ(out.pixels[4], 2); // Pixel 1 R
}
...
TEST(NativeCPU, CropBottomRightPixel) {
  ...
  EXPECT_EQ(out.pixels[0], 4);
  EXPECT_EQ(out.pixels[1], 4);
  EXPECT_EQ(out.pixels[2], 4);
...
TEST(NativeCPU, ConvolutionOp) {
  ...
  EXPECT_EQ(out.pixels[16], 100);
  EXPECT_EQ(out.pixels[17], 100);
  EXPECT_EQ(out.pixels[18], 100);
  EXPECT_EQ(out.pixels[19], 255);
}
...
TEST(NativeCPU, BlurOp) {
  ...
  // Initialize image with 100 for ALL channels (R, G, B, A)
  img.pixels.assign(10 * 10 * 4, 100);

  // Initialize out with 100 for ALL channels (R, G, B, A)
  out.pixels.assign(10 * 10 * 4, 100);
  ...
  EXPECT_EQ(out.pixels[0], 100); // R
  EXPECT_EQ(out.pixels[3], 100); // A (Blurred)
}
...
TEST(SYCL_CPU, BlurOp) {
  ...
  EXPECT_EQ(out.pixels[0], 100);
  EXPECT_EQ(out.pixels[3], 100);
}
